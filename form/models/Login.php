...
class Login extends Model
{
    ...
    public function verifyEmail($email,PDO $pdo){
        $params = [];
        $sql = "select first_name, login_id from login where email = '$email' limit 1";
    ...
    public function updatePassword($id,$new_pass, PDO $pdo){
        $params = [];
        $sql = "update login set password = '$new_pass' where login_id = $id limit 1";
        $stmt = $pdo->prepare($sql);

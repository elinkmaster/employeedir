...
function validateForm()
{
    ...
    {
        $('.status-msg-bar').html('<p class="bg-danger">Please enter the Form Title</p>');
        $('#survey_name').css('border', '1px solid #dc3545');
        alert('Please enter the Form Title');
        return false;
    } else {
        //Check if survey name exist
        $.post('form_edit.php?func=checkSurveyNameExist',
                { 
                    survey_id: $('#form_id').val(),
                    survey_name: $('#survey_name').val()
                },
                function(data) {
                    var d = JSON.parse(data);
                    if(d == false) {
                        $('#survey_edit_form').submit();
                    } else {
                        $('.status-msg-bar').html('<p class="bg-danger">Form Title is already exist</p>');
                        $('#survey_name').css('border', '1px solid #dc3545');
                        alert('Form Title is already exist');
                    }
                }
            );
        //end

...
<div class="container-fluid">
    <div class="row">
        <div class="col-md-3">
            ...
            ?>
                <div>
                    <button data-id="<?php echo $t['id'] ?>" data-status="<?php echo $t['status']; ?>" type="button" class="btn btn-mn btn-<?php echo $t['id'] == 8 ? "danger" : "primary" ?> timer_s"><?php echo $t['desc'] ?></button>
                </div>
                <br>
            <?php    
                endif;
            endforeach;
            ?>
                <!-- <button data-id="0" data-status="1" type="button" class="btn btn-mn btn-danger timer_s">Time Out</button> -->
        </div>
        <div class="col-md-6">
            <div class="row">
                <div class="col-xs-2 bg-info" style="font-weight: bold; text-align: center;">Break Type</div>
                <div class="col-xs-4 bg-info" style="font-weight: bold; text-align: center;">Time Start</div>
                <div class="col-xs-4 bg-info" style="font-weight: bold; text-align: center;">Time Stop</div>
            </div>
            <?php
            /*
            ...
            */
            ...
            ?>
            <div class="row">
                <div class="col-xs-2<?php echo $b->bi_status == 1 ? " bg-danger" : " bg-info" ?>"><label><?php echo $b->break_name ?></label></div>
                <div class="col-xs-4<?php echo $b->bi_status == 1 ? " bg-danger" : " bg-info" ?>"><label><?php echo date("M d Y - h:i:s A", strtotime($b->time_start)) ?></label></div>
                <div class="col-xs-4<?php echo $b->bi_status == 1 ? " bg-danger" : " bg-info" ?>"><?php echo is_null($b->time_stop) ? "On going - <label id='_".$b->bi_id."_hours'>".$diff->format( '%H' )."</label>:<label id='_".$b->bi_id."_minutes'>".$diff->format( '%I' )."</label>:<label id='_".$b->bi_id."_seconds'>".$diff->format( '%S' )."</label>" : "<label>".date("M d Y - h:i:s A", strtotime($b->time_stop))."</label>" ?></div>
            </div>
            <?php
            if(is_null($b->time_stop)):
            ?>
